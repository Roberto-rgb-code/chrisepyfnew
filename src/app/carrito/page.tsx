...
function CartContent() {
  ...
  const total = appliedPromo ? appliedPromo.totalAfterDiscount : subtotal;
  ...
  const applyPromoCode = async () => {
    ...
    try {
      const res = await fetch('/api/promo/validate', {
        ...
        body: JSON.stringify({ code: promoInput, subtotal, items: cart }),
      });
      ...
      showToast(`Código ${data.code} aplicado`, 'success');
    } catch {
      showToast('Error al validar código', 'error');
    } finally {
          ...
          <div className="bg-white rounded-2xl shadow-sm p-5 sm:p-6 border border-gray-100 lg:sticky lg:top-24">
            ...
            <div className="mb-4">
              ...
              <div className="flex gap-2">
                <input
                  ...
                  className="flex-1 px-3 py-2.5 border border-gray-200 rounded-xl text-sm uppercase"
                />
                <button
                  type="button"
              ...
              {appliedPromo && (
                <p className="text-xs text-green-600 mt-2 font-medium">
                  ✓ {appliedPromo.code} — −${appliedPromo.discountAmount} MXN
                </p>
              )}
            </div>
            ...
            <div className="space-y-3 mb-6">
              <div className="flex justify-between text-gray-600">
                <span>Subtotal</span>
                <span className="font-semibold text-gray-900">${subtotal} MXN</span>
              </div>
              {appliedPromo && (
                <div className="flex justify-between text-green-600">
                  <span>Descuento ({appliedPromo.code})</span>
                  <span className="font-semibold">−${appliedPromo.discountAmount} MXN</span>
                </div>
              )}
              <div className="flex justify-between text-gray-600">
                <span>Envío</span>
              ...
              <div className="border-t pt-3 flex justify-between">
                ...
                <span className="text-2xl font-bold text-green-600">${total} MXN</span>
              </div>
            </div>
